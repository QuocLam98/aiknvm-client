...
const getBot = async () => {
  defaultBot.value = import.meta.env.VITE_DEFAULT_BOT
  try {
    const res = await axios.get(`${urlServer}/get-bot/` + defaultBot.value);
...
const sendMessage = async () => {
  ...
  formData.append('bot', defaultBot.value || '')
  ...
  messages.value.push({
    ...
    fileUser: previewFile.value ? previewFile.value : '',
  ...
  try {
    ...
    nextTick(() => {
      typeWriterEffect(message, contentBot); // contentBot là text gốc chưa render
    })
  ...
  <div class="min-h-[calc(100vh-5rem)] relative">
    <div class="bg-zinc-50" style="height: calc(10px - 7.6rem + 105vh);">
      ...
      <div v-if="getBotData" ref="chatContainer"
        ...
        <!-- Hiển thị thông tin bot nếu không có tin nhắn -->
        ...
        <div v-for="(message, index) in messages" :key="index"
          ...
          <!-- Nếu là user và fileUser có hình -->
          <div v-else
            ...
            <!-- Word -->
            <figure
              v-else-if="message.fileType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'">
              <img
          ...
          <div class="chat-bubble">
            ...
            <!-- Các biểu tượng "Copy", "Like", "Dislike" nằm dưới tin nhắn -->
...
.chat-actions {
  ...
  gap: 8px;
  margin-top: 4px;
  /* Khoảng cách giữa chat-bubble và các nút */
}
