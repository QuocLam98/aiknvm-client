...
const validateAttachment = (file: File) => {
  ...
  if (!file.type.startsWith('image/')) {
    showToastError('Chỉ chấp nhận ảnh!', TOAST_DEFAULT_DURATION)
    return false
  }
...
const sendMessage = async () => {
  ...
  if (botImage.value === botPremium && previewFile.value) {
      ...
      <div class="bg-white shadow-xl flex flex-col py-2 px-3 border-l border-t rounded-lg border-base-300 w-full">
        <div class="flex flex-col w-full">
          ...
          <div class="flex w-full items-center">
            ...
            <label class="ml-2 cursor-pointer" v-if="botImage === botPremium">
